...
### `repomix-explorer`
**用途：** 把整個 codebase 壓縮成單一 AI 最佳化檔案（XML/Markdown），讓 Claude 一次看懂整個專案結構、API endpoint、依賴關係。內建 token 壓縮（減少約 70%）、安全掃描（避免 API key 外洩）、Git 感知（自動排除 .gitignore 檔案）。
**觸發：** 說「分析這個 repo」、「整個專案結構是什麼」、「找所有 API endpoint」、「把這個專案打包給 AI」
```
「分析 facebook/react 這個 repo，找出核心架構」
「把這個專案打包給 AI 分析」
「進入這個新專案前，先讓 Claude 看懂整體結構」
「分析 https://github.com/someuser/repo 的架構，不用 clone」
```
**主要場景：**
- 進入陌生專案前：`repomix` → 貼給 Claude 做全局分析
- 重構前影響分析：先產出全局 map，再問「這個改動影響哪些地方」
- 分析遠端 repo：`repomix --remote https://github.com/user/repo`（不需 clone）

> ✅ 已安裝：`repomix v1.14.0`（`npm install -g repomix`）
