...
復查觸發（2026-07-17 補）：
- **升級條件**（→ ✅ 裝）：npx 試用排盤結果與既有八字排盤（life-chart-engine/sxtwl 系）交叉驗證一致 → 值得深度整合（如 fate/ 專案接入 @openfate/bazi-engine）
- **放棄條件**（→ ❌ 不裝）：試用排盤結果與既有系統有出入，或閉源引擎無法驗證準確性的疑慮無法解除 → 放棄整合，維持現有八字排盤
