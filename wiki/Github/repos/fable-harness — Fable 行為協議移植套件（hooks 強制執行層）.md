...
⏳ **觀望整包、單挑 verify_gate 試裝** — 制度文件層與本系統重疊 95%，整包安裝製造雙源衝突（兩套 CLAUDE.md 路由/兩套派工規則打架）。唯 verify_gate.py 填補真實缺口，建議先在單一子專案試跑一週再決定全域化。首日 repo，可先 watch 其 issue 區成熟度。

復查觸發（2026-07-17 補）：
- **升級條件**（→ ✅ 裝）：verify_gate.py 單一子專案試跑一週後表現穩定（無誤擋、無 fail-open 漏判）→ 決定全域化
- **放棄條件**（→ ❌ 不裝）：試跑期間出現誤擋卡死，或 issue 區反映重大 bug → 放棄採用，回到純自律 R10/R12
