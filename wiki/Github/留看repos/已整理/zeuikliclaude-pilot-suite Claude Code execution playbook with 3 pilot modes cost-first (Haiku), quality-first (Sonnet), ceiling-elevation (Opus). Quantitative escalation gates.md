---
title: "zeuikli/claude-pilot-suite: Claude Code execution playbook with 3 pilot modes: cost-first (Haiku), quality-first (Sonnet), ceiling-elevation (Opus). Quantitative escalation gates."
source: "https://github.com/zeuikli/claude-pilot-suite?fbclid=IwY2xjawRr-kNleHRuA2FlbQIxMABicmlkETFac3VDaGlnSmltc0NpZ09Wc3J0YwZhcHBfaWQQMjIyMDM5MTc4ODIwMDg5MgABHgPSBcSOSsLlVKX3XzxxUah7tikCieuKom7z1MYSxqL3lq6pJS8JXHfT1NuE_aem_ps2ZvLehjMxYJpA9dEUfdQ"
author:
published:
created: 2026-05-09
description: "Claude Code execution playbook with 3 pilot modes: cost-first (Haiku), quality-first (Sonnet), ceiling-elevation (Opus). Quantitative escalation gates. - zeuikli/claude-pilot-suite"
tags:
  - "clippings"
---
## Claude Pilot Suite
...
**結論：不需要安裝，但 Haiku Pilot 有一個功能值得單獨借鑑。**

---

## 你目前已有的能力對比

|claude-pilot-suite 提供|你已有的替代方案|
|---|---|
|Sonnet Pilot（品質優先）|CLAUDE.md 中已有 Sonnet/Opus 分工表 + Ruflo|
|Opus Pilot（提升 Opus 上限）|`/fast` 切換 Opus + recursive-mode 九個子技能|
|規則目錄（git、subagent、格式）|CLAUDE.md 已完整定義 + 76 個 Skills|
|升級閘門（同問題失敗 3 次才升級）|Ruflo autopilot + recursive-debugging 已處理|

## 唯一的缺口

**Haiku Pilot** — 你目前的 CLAUDE.md 只列出 Sonnet / Opus，完全沒用到 Haiku 4.5。claude-pilot-suite 的 Haiku Pilot 主張簡單任務用 Haiku 可省 70–85% 成本。

但你用的是 Claude Code（訂閱制），**不是按 token 計費的 API**，所以成本省不省跟你沒關係。

## 最終建議

**跳過這個 repo**。它針對的是 API 按量付費用戶的成本控制問題，你的環境已用訂閱制 + Ruflo 解決了同樣的模型分派需求，裝了只是多一套重複的規則文件。