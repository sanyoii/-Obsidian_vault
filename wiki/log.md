...
### 2026-07-31 | LINT
- `2026-07-31 11:08 | LINT | wiki/Github/repos/ 孤兒檔清理 → _index.md 190→184 篇` | 逐字讀證 13 個未索引檔後分四類處置：刪 5（3 個 Web Clipper 空殼＋1 個 knowledge-work-plugins 重複剪貼＋zhaoolee Chrome 插件清單 52KB）、改名 3（2 個檔名超 280 UTF-8 bytes 造成 git-bash 工具隱形的檔＋1 個「github cover」名不符實檔）、搬 1（「Github 留看 Repo 整理」meta 清單移出 repos/ 至 Github/ 根層並修「已安裝工具」引用）、補索引 7 行；原判「刪除」的 cli-printing-press 與 open-slide 經逐字讀後翻案保留；建立檔名 ≤280 bytes 硬規則（實測 255 可見／296 隱形）

### 2026-07-30 | COMPILE
- `2026-07-30 21:15 | COMPILE | raw/sources/lehman-manuals/examiner-report/Vol1(239頁) → wiki/Quant-Trading/Examiner報告-Vol1-風險治理對讀.md` | 第三次嘗試才成功（前兩次 agent 皆倒在 API 529）；106,487 字元／18 節／16 實例／24 組對讀／15 題自測；**核心價值是九問答覆表**——把 CRR 手冊末章的 9 個查核問句逐一拿到 Vol 1 驗證：3 題有直接答案（彙總掩蔽的機制是排除清單與平均化而非 Top N；超限處理為調高 3 次/豁免≥1 次/不執行 1 項/**降風險 0 次**；超限帳齡靠 2008-01 回溯生效歸零）、4 題部分涵蓋、**2 題明示 Vol 1 未涵蓋並指出應查 Vol 2 或 Vol 9**（對帳、孤兒帳戶）；agent 誠實回報四項限制：Vol 9 附錄未讀、頁碼換算式（報告內頁＝PDF頁−44）僅用兩錨點驗未逐頁核、人事類記載因證詞互相矛盾而並列不採信任一方、未更新 log（依派工單禁改既有檔，由編排者補登）；反向連結 6/6 實測存活；_index 401→402
